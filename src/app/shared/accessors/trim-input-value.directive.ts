...
/**
 * stolen from:  https://github.com/khashayar/ng-trim-value-accessor
 * but not working for material autocomplete
...
export class TrimInputValueDirective extends DefaultValueAccessor {
  ...
  @HostListener('input', ['$event.target.value']) trimOnChange = (val: string) => {
    this.onChange(val.trim());
  }

  @HostListener('blur', ['$event.target.value']) trimOnBlur(val: string) {
    this.writeValue(val);
  }
